...
WHEREAS, the Seed Revenue-Based Financing raise targets **USD $400,000** in additional capital (following an initial **USD $38,000 pre-seed** already contributed);  
WHEREAS, Pre-Seed contributors receive a **1.15x weighting multiplier** applied to their contributed principal when calculating revenue distribution shares (a one‑time uplift applied to the entire $38,000 pre-seed tranche and not to later Seed capital);  
...
1.3 **"Weighted Contribution"** means each Investor’s contributed principal in USD equivalent, multiplied by 1.15 if (and only if) that principal (or a portion thereof) was part of the initial Pre-Seed $38,000 tranche; later Seed contributions are weighted at 1.00.  
...
1.12 **"Pre-Seed Multiplier"** means the 1.15 factor applied to the entire $38,000 Pre-Seed tranche only (not to later Seed capital).
...
1.14 **"2× Review Threshold"** means the point at which an individual Investor's cumulative Distributions received under this Agreement equal two (2) times that Investor's Original Contributed Principal.

---

### 2. Financing and Revenue Share
2.1 **Raise Structure**: Investors contribute Bitcoin toward a target Seed raise of **USD $400,000** on Angor. An earlier **USD $38,000 Pre-Seed** has been completed; the entire $38,000 tranche is subject to the Pre-Seed Multiplier (later Seed capital is unweighted).  
...
7.2 **Termination Events**: (a) Permanent discontinuation of the Nostria premium offering (decision of Nostria team); (b) Regulatory prohibition of revenue distribution; (c) Fraud or willful misconduct (Nostria team may elect to cease distributions); (d) Insolvency or inability to continue operations. Investor consultation may occur but is not required for a termination decision.  
...
7.5 **2× Review (Team-Directed)**: Hitting the 2× Review Threshold does not automatically terminate or suspend Distributions. It triggers an internal review by the Nostria team (which may, but is not required to, solicit informal investor feedback). The team may decide to: (a) continue unchanged; (b) adjust the Investor Pool percentage prospectively; (c) initiate a structured wind‑down of future Distributions; or (d) explore a transition to an alternate participation mechanism (including potential equity conversion if later incorporated). Any change that alters economic terms requires amendment per Section 10.2. Absence of a team decision results in continuation under existing terms.  
...
10.3 **Assignment / Transfer**: Revenue rights may be transferred upon prior written notice to (and acknowledgment by) the Nostria Coordinators (or designated operations contact) so that records and payout addresses can be updated. Until the update is confirmed, distributions continue to the previously registered payout destination. Any public promotion or creation of a speculative secondary market remains prohibited.  
